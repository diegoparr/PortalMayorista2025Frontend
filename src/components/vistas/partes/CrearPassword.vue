<template>
  <div>
    <div v-if="config">
      <div>
        <div style="color: #ec6920">Cree una nueva Contraseña</div>
        ********** <v-btn color="primary" @click.stop="modal=true" slot="activator" fab small><v-icon>fa fa-edit</v-icon></v-btn>
      </div>
    ...
    <v-layout row justify-center>
      <v-dialog v-model="modal" persistent max-width="500px">
        <form v-on:submit.prevent="crearPassword">
          <v-card>
            <v-card-title>
              <span class="headline">Crear una contraseña</span>
            </v-card-title>
            <v-card-text>
            ...
            <v-card-actions>
              ...
              <v-btn color="success" :onclick="crearPassword" type="submit">Enviar</v-btn>
            </v-card-actions>
          </v-card>
        </form>
      </v-dialog>
  ...
  export default {
    ...
    methods: {
      ...
      crearPassword() {
        ...
        console.log('USER', this.getUsuario);
        this.getAppServices().postMethodWithBearer('api/avanzamas/crearPassword', this.data, token)
          .then(response => {
            //boton_registrar.button('reset');
            yo.$toastr('success', "Contraseña creada", "Acción exitosa");
            ...
            if (errors.status === 422) {
              ...
              yo.$toastr('error', errors.body.mensaje, "Campos incorrectos");
            //boton_registrar.button('reset');
          });
      }
    },
